...
public final class MarsCipherAlgorithm  {

    private static final int MASK_255 = 0xff;
    protected byte[] key;
    protected int[] K;

    public MarsCipherAlgorithm(byte[] key) {
        this.key = key;
        K = expandKey(key);
    }
    ...
    public byte[] encrypt(byte[] in) {
        ...
        //создаем массив данных(32)
        int[] data = new int[in.length / 4];
        int off = 0;


        for (int i = 0; i < data.length; i++) {
            data[i] = (in[off++] & MASK_255) | ((in[off++] & MASK_255) << 8) | ((in[off++] & MASK_255) << 16) | ((in[off++] & MASK_255) << 24);
        }

        int firstWord = data[0] +K[0];
        int secondWord = data[1] + K[1];
        int thirdWord = data[2] + K[2];
        int fourthWord = data[3] + K[3];

        //прямое смешивание
        int swapWord;
        for (int i = 0; i <= 7; i++) {
            secondWord ^= S[firstWord & MASK_255];
            secondWord += S[(rightRotate(firstWord, 8) & MASK_255) + 256];
            thirdWord += S[rightRotate(firstWord, 16) & MASK_255];
            fourthWord ^= S[(rightRotate(firstWord, 24) & MASK_255) + 256];

            firstWord = rightRotate(firstWord, 24);

            if (i == 1 || i == 5){
                firstWord += secondWord;
            }
            if (i == 0 || i == 4){
                firstWord += fourthWord;
            }
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            eout = eFunction(firstWord, K[2 * i + 4], K[2 * i + 5]);

            firstWord = leftRotate(firstWord, 13);
            secondWord += eout[0];
            thirdWord += eout[1];
            fourthWord ^= eout[2];
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            secondWord = secondWord ^ S[(firstWord & MASK_255) + 256];
            thirdWord = thirdWord - S[leftRotate(firstWord, 8) & MASK_255];
            fourthWord = fourthWord - S[(leftRotate(firstWord, 16) & MASK_255) + 256];
            fourthWord = fourthWord ^ S[leftRotate(firstWord, 24) & MASK_255];
            firstWord = leftRotate(firstWord, 24);
        ...
        firstWord = firstWord - K[36];
        secondWord = secondWord - K[37];
        thirdWord = thirdWord - K[38];
        fourthWord = fourthWord - K[39];
        ...
        for (int i = 0; i < in.length; i++) {
            res[i] = (byte) ((data[i / 4] >>> (i % 4) * 8) & MASK_255);
        }
    ...
    public byte[] decrypt(byte[] in) {
        ...
        int firstWord = data[0] + K[36];
        int secondWord = data[1] + K[37];
        int thirdWord = data[2] + K[38];
        int fourthWord = data[3] + K[39];
        ...
        for (int i = 7; i >= 0; i--) {
            ...
            fourthWord = fourthWord ^ S[(rightRotate(firstWord, 8) & MASK_255)];
            fourthWord = fourthWord + S[(rightRotate(firstWord, 16) & MASK_255) + 256];
            thirdWord = thirdWord + S[rightRotate(firstWord, 24) & MASK_255];
            secondWord = secondWord ^ S[(firstWord & MASK_255) + 256];

            if (i == 2 || i == 6) firstWord = firstWord + fourthWord;
            if (i == 3 || i == 7) firstWord = firstWord + secondWord;
        ...
        for (int i = 15; i >= 0; i--) {
            ...
            firstWord = rightRotate(firstWord, 13);
            eout = eFunction(firstWord, K[2 * i + 4], K[2 * i + 5]);

            thirdWord = thirdWord - eout[1];

            if (i < 8) {
                secondWord = secondWord - eout[0];
                fourthWord = fourthWord ^ eout[2];
            } else {
                fourthWord = fourthWord - eout[0];
                secondWord = secondWord ^ eout[2];
            }
        ...
        for (int i = 7; i >= 0; i--) {
            ...
            if (i == 0 || i == 4) firstWord = firstWord - fourthWord;
            if (i == 1 || i == 5) firstWord = firstWord - secondWord;

            firstWord = leftRotate(firstWord, 24);

            fourthWord = fourthWord ^ S[(rightRotate(firstWord, 24) & MASK_255) + 256];
            thirdWord = thirdWord - S[rightRotate(firstWord, 16) & MASK_255];
            secondWord = secondWord - S[(rightRotate(firstWord, 8) & MASK_255) + 256];
            secondWord = secondWord ^ S[firstWord & MASK_255];


        }
        firstWord = firstWord - K[0];
        secondWord = secondWord - K[1];
        thirdWord = thirdWord - K[2];
        fourthWord = fourthWord - K[3];
    ...
    private int[] eFunction(int in, int firstKey, int secondKey) {

        int[] tmp = new int[3];
        int M, L, R;
        M = in + firstKey;
        L = S[M & 0x000001ff];
        R = leftRotate(in,13) * secondKey;
        R = leftRotate(R,5);
        L = L ^ R;
        M = leftRotate(M, R);
        R = leftRotate(R,5);
    ...
    private int[] expandKey(byte[] key) {
        ...
        for (int i = 0; i < key.length; i++)
            t[i / 4] |= (key[i] & MASK_255) << (i * 8);
        t[n] = n;

        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 15; i++)
                t[i] ^= leftRotate(t[(i + 8) % 15] ^ t[(i + 13) % 15], 3) ^ (4 * i + j); //document error
    ...
    private int leftRotate(int val, int pas) {
        return (val << pas) | (val >>> (32 - pas));
    }

    private int rightRotate(int val, int pas) {
        return (val >>> pas) | (val << (32 - pas));
    }

    private int generateMask (int x){
        int m;
